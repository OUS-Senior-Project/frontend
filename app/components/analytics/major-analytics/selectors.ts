...
export function selectWeightedGpaByMajor(data: MajorCohortRecord[]) {
  ...
  data.forEach((record) => {
    if (!majorMap[record.major]) majorMap[record.major] = { total: 0, count: 0 };
  ...
  return Object.entries(majorMap)
    .map(([major, value]) => ({ major, avgGPA: Math.round((value.total / value.count) * 100) / 100 }))
...
export function selectWeightedCreditsByMajor(data: MajorCohortRecord[]) {
  ...
  return Object.entries(majorMap)
    .map(([major, value]) => ({ major, avgCredits: Math.round(value.total / value.count) }))
    .sort((a, b) => b.avgCredits - a.avgCredits);
}

export function selectCohortRowsByMajor(data: MajorCohortRecord[], metric: 'avgGPA' | 'avgCredits') {
  const majors = Array.from(new Set(data.map((record) => record.major)));

  return majors.map((major) => {
    const row: Record<string, string | number> = { major };
    cohorts.forEach((cohort) => {
      const match = data.find((record) => record.major === major && record.cohort === cohort);
